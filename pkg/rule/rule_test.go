...
func TestCharToRule(t *testing.T) {
	...
	tests := []testCase{
		...
		{"test ", "$", "$t $e $s $t $\\x20"},
		{" test", "^", "^\\x20 ^t ^e ^s ^t"},
		{"爱test", "$", "$\\xE7 $\\x88 $\\xB1 $t $e $s $t"},
		{"tset爱", "^", "^t ^s ^e ^t ^\\xB1 ^\\x88 ^\\xE7"},
	}
...
func TestCharToIteratingRule(t *testing.T) {
	...
	tests := []testCase{
		...
		{"爱test", "i", 1, "i1\\xE7 i2\\x88 i3\\xB1 i4t i5e i6s i7t"},
		{"爱test", "i", 2, "i2\\xE7 i3\\x88 i4\\xB1 i5t i6e i7s i8t"},
		{"test123", "i", 3, "i3t i4e i5s i6t i71 i82 i93"},
		{"a b", "i", 0, "i0a i1\\x20 i2b"},
	}
...
func TestFormatCharToRuleOutput(t *testing.T) {
	...
	tests := []testCase{
		{"$t $e $s $t", "$t $e $s $t"},
		{"^t ^e ^s ^t", "^t ^e ^s ^t"},
		{"$\\xE7 $\\x88 $\\xB1 $t $e $s $t", "$\\xE7 $\\x88 $\\xB1 $t $e $s $t"},
		{"^\\xE7 ^\\x88 ^\\xB1 ^t ^e ^s ^t", "^\\xE7 ^\\x88 ^\\xB1 ^t ^e ^s ^t"},
		{"$t $e $s $t $\\x20", "$t $e $s $t $\\x20"},
	}
...
func TestFormatCharToIteratingRuleOutput(t *testing.T) {
	...
	tests := []testCase{
		...
		{1, "i1\\xE7 i2\\x88 i3\\xB1 i4t i5e i6s i7t", "i1\\xE7 i2\\x88 i3\\xB1 i4t i5e i6s i7t"},
		{2, "i2\\xE7 i3\\x88 i4\\xB1 i5t i6e i7s i8t", "i2\\xE7 i3\\x88 i4\\xB1 i5t i6e i7s i8t"},
...
func TestAppendRules(t *testing.T) {
	...
	tests := []testCase{
		...
		{map[string]int{"test ": 1}, "append", map[string]int{"$t $e $s $t $\\x20": 1}},
	}
...
func TestPrependRules(t *testing.T) {
	...
	tests := []testCase{
		...
		{map[string]int{" test": 1}, "prepend", map[string]int{"^t ^s ^e ^t ^\\x20": 1}},
	}
